...
namespace HealthCareAgenticAI.Services
{
        ...
        public async Task<ResponseModel> MainLogic(RequestModel request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (!request.IsThisFristRequestForClaim &&
                    !(request.FeedbackClaimId || request.FeedbackInstructions || request.FeedbackSummary))
                {
                ...
                if (evaluationResponse!.FeedbackInstructions && evaluationResponse.FeedbackSummary && evaluationResponse.FeedbackClaimId)
                {
