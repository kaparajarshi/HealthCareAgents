...
namespace HealthCareAgenticAI.Helpers
{
    public static class PromptText
    {
        public static readonly string EvaluationPrompt = $@"Return STRICT JSON that matches the structure:
                                                ...
                                                - Feedback for {RagLabels.ReferenceLabel}: 'true' if matches {CustomRegex.ClaimIdStrict()} and should not be the value of {RagLabels.ClaimLabel}. Or couldn't extract, else 'false'
                                                - Feedback for {RagLabels.InstructionsLabel}: 'true' if it has concise, actionable steps and contains action words : {string.Join(", ", InstructionKeywords.Actions)}. Or couldn't extract, else 'false'
        ...
        public static readonly string LlmPrompt = $@"Return STRICT JSON that matches the structure:
                                                ...
                                                - {RagLabels.ReferenceLabel}: include only if it matches {CustomRegex.ClaimIdStrict()}, else null. If it matches multiple, return the first match and should not be the value of {RagLabels.ClaimLabel}.
                                                - {RagLabels.InstructionsLabel}: concise, actionable steps only, else null. It should contains these actions/words: {string.Join(", ", InstructionKeywords.Actions)}
